...
def scan_card():
    ...
    if uid not in db:
        db[uid] = {"scans": 1, "level": "ROOKIE", "last_seen": datetime.now().strftime("%H:%M:%S"), "has_nft": False}
        status = "new user"
    else:
        db[uid]["scans"] += 1
...
# NEW ENDPOINT: Marks the user as having minted the NFT
...
def nft_minted():
    ...
    try:
        ...
        if uid in db:
            db[uid]['has_nft'] = True
            save_db(db)
            print(f"[nft] card {uid} successfully minted NFT and saved to db")
            return jsonify({"status": "success", "message": "nft status updated"})
        else:
            return jsonify({"error": "card not found in database"}), 404
